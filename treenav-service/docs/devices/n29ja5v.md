...
AirPlay functionality is not supported when a proxy is enabled on your network. Some features may appear to work, but consistent performance cannot be guaranteed.
...
| Port        | TCP or UDP | Direction | Service or protocol name                                | Service name | Used by |
| ----------- | ---------- | --------- | ------------------------------------------------------- | ------------ | ------- |
| 80          | TCP        | Inbound   | Hypertext Transfer Protocol (HTTP)                      | http         | AirPlay |
| 443         | TCP        | Inbound   | Secure Sockets Layer (SSL or HTTPS)                     | https        | AirPlay |
| 319, 320    | UDP        | Inbound   | Precision Time Protocol (PTP)                           | ptp          | AirPlay |
| 554         | TCP/UDP    | Inbound   | Real Time Streaming Protocol (RTSP)                     | rtsp         | AirPlay |
| 3689        | TCP        | Inbound   | Digital Audio Access Protocol (DAAP)                    | daap         | AirPlay |
| 5353        | UDP        | Inbound   | Multicast DNS (MDNS)                                    | mdns         | AirPlay |
| 7000        | TCP        | Inbound   | AirPlay HTTP server                                     | http         | AirPlay |
| 32768-60999 | TCP/UDP    | Outbound  | Ephemeral port range required between subnets for share | http         | AirPlay |

Configuration 

Configuring a Cisco device with AirPlay requires an internet connection with access to the Webex cloud, in order to receive the AirPlay provisioning information.

Internet connection is necessary for first-time configuration, configuration after factory reset, and configuration after software downgrade.

To configure AirPlay on a Cisco device, access the device's web interface or Control Hub. See the [Device Configurations article](https://help.webex.com/en-US/article/n5pqqcm/Device-Configurations-for-Room-and-Desk-Devices-and-Webex-Boards) for more information. Then, select and set the following configurations.
...
The AirPlay beacon is a Bluetooth low energy information broadcast sent using Apple's vendor specific BLE Header. It contains the video device's IPv4 address. The Apple device will read the beacon and then probe the IP address to see if it can reach AirPlay-capable devices. Then it pulls the device name over the IP network and populates the list of AirPlay targets on the Apple device. Non-routeable devices such as a neighbor's device or any other firewalled device, will not be displayed. Users on a cellular network or guest WiFi will not be able to use this functionality either.
...
**Save** your changes before closing the web interface or Control Hub. Detailed information about each setting is available in the [Cisco collaboration devices API Reference Guide](https://www.cisco.com/c/dam/en/us/td/docs/telepresence/endpoint/roomos-111/api-reference-guide-roomos-111.pdf).

See [this article](https://help.webex.com/article/ndlsg3c/Wireless-sharing-with-AirPlay) about how to share wirelessly with AirPlay on Cisco devices, and end-user settings.
