---
title: "Requirements for Business-to-Business (B2B) SIP calls to and from the Webex cloud"
product: "Hybrid Services"
article_id: "7ej8gq"
url: "https://help.webex.com/en-us/article/7ej8gq"
last_updated: "2025-08-05"
description: "You can use these requirements for business-to-business (B2B) SIP calls to and
from the Webex cloud across the Internet. They apply if your users make or
receive calls between cloud clients (Webex App or Webex-registered devices) and
third-party enterprises or services that use SIP."
...
- The destination SBC must present a valid server certificate (not expired).
...
- Present a valid TLS certificate that contains the SBC's FQDN in either the CN or SAN DNS records.
...
- Present a TLS certificate signed by a trusted public root Certificate Authority (CA). For the list of trusted root CAs, see [ Supported Certificate Authorities for Webex Hybrid Services](https://help.webex.com/0q4utq).
- Have a DNS PTR record configured for the SBC's FQDN.

Additional notes to keep in mind:

- We don't support cascading meetings from on-premises video conferencing bridges (Cisco TelePresence Server or Cisco Meeting Server) or cloud conferencing systems to the Webex Video Platform.
...
- You can test your SBC's DNS SRV records and connectivity using the [Cisco TAC Collaboration Solutions Analyzer ](https://cway.cisco.com/tools/CollaborationSolutionsAnalyzer/).
...
- The originating SBC must be capable of using the \_sips.\_tcp DNS SRV record corresponding to the subdomain in the dialed URI's host portion to locate the FQDN of the Webex SIPS server, and resolving the A record for the FQDN to determine the IPv4 address to connect to.
...
- The originating SBC must be able to connect to the Webex SIPS server on the IP address determined from the DNS lookups, and be able to negotiate SIP over TLS.
...
- Present a valid TLS certificate signed for both client and server usage.
...
- Establish a mutual TLS connection to the Cisco Collaboration Cloud on TCP port 5062.
  ...
  - If you are using a Cisco VCS-Expressway or Cisco Expressway-Edge as the SBC, this can be done using a custom DNS zone for Webex which has the **TLS verify mode** and **Modify DNS request** options set to **On**, and the **TLS verify subject name** and **Domain to search for**fields set to **callservice.ciscospark.com**.
...
- [ Network Requirements for Webex Services ](https://help.webex.com/WBX000028782) (**Cloud only**)
- [ Deployment guide for Video Mesh](https://help.webex.com/2r5gv7) (**Video Mesh nodes to keep media on-premises**)
