---
title: "Known issues and limitations for Webex Scheduler for Microsoft Outlook"
product: "Webex Meetings"
article_id: "n194fs"
url: "https://help.webex.com/en-us/article/n194fs"
last_updated: "2025-10-22"
description: "Learn about known issues and limitations for the Webex Scheduler."
tags: ["webex-meetings"]
...
- The minimum system requirements for the Webex Scheduler are:
  ...
  - Microsoft Office 2016 for Mac not supported
- For Webex meetings scheduled with the Webex Scheduler, you see the Join button on Webex video devices only if the Webex Hybrid Calendar Service is enabled and configured according to the instructions in [Make it easier for video devices to join meetings with OBTP](https://help.webex.com/yvz1kw/).
...
- If you schedule a meeting using the Webex Scheduler and change the corresponding Outlook event while your meeting is in progress, the changes you made won't sync to your Webex site.
...
- Adding a Webex meeting to a single occurrence in a recurring calendar event that was previously scheduled in Outlook isn't supported when you schedule meetings on behalf of others.
...
- You can't schedule recurring webinars using Webex Scheduler.
...
- To bypass login when using the Webex Scheduler with a Microsoft SSO token in Outlook, ensure your Outlook client is upgraded to a version newer than the 2016 MSO (Version 2412 Build 16.0.18324.20092) 32-bit. This is because older versions do not support the SSO token API needed for seamless integration.
- Webex Scheduler on mobile doesn't support the 'schedule on behalf of' feature.
- If the email meeting invitation already contains a meeting link, including a Personal Room link, it prevents the Webex Scheduler from adding a new meeting.
...
- You may need to configure your network to allow the Webex Scheduler to access these URLs:
  - \*.webex.com
  - appsforoffice.microsoft.com—for OfficeJS lib
- You might not be able to schedule meetings for other hosts using a shared folder (delegate scheduling) in Outlook for Windows. The options appear dimmed due to a Microsoft on-premises Exchange limitation. [You can still schedule Webex meetings for others using their Webex host account with the Webex Scheduler](https://help.webex.com/article/nlx8vr9/). If you schedule meetings for other hosts, the Outlook meeting invite appears to be from you instead of the host you scheduled the meeting for. The host that you schedule the meeting for doesn’t receive a meeting invite in Outlook unless you add them as an attendee in Outlook.
- On Mac Outlook, meeting invitations may not show the correct formatting for lists due to a Microsoft Outlook Office.js limitation.
...
- Webex Scheduler adds the meeting details to your Outlook message in HTML. If your Outlook messages use the Rich Text Format (RTF), meeting invites may not appear correctly when the meeting invite details convert to RTF. Use HTML for your Outlook messages to see the correct formatting.
- You can't schedule webinars using Webex Scheduler.
- Webex Scheduler doesn't support shared mailboxes (preview) in Outlook.

The following limitations only apply to organizations that have NOT deployed [Hybrid Calendar for Microsoft Exchange](https://help.webex.com/article/mwka5l/).
