---
title: "Configure On-Premises Devices for Webex App Users"
product: "Devices"
article_id: "poqjhk"
url: "https://help.webex.com/en-us/article/poqjhk/Configure-On-Premises-Devices-for-Cisco-Webex-Teams-Users"
last_updated: "2025-11-27"
description: "Configure your on-premises devices for users to connect to using ultrasound.
Users can share content wirelessly in local meetings, join meetings, make or
answer calls using the on-premises devices."
tags: ["devices", "webex-suite"]
source: "help.webex.com"
---

# Configure On-Premises Devices for Webex App Users

## Allow Users to Connect to On-Premises Devices

When you have on-premises devices, you can configure them for your users to connect to. When their computer is on the same Local Area Network (LAN) as the device they can connect and do the following:

- Call somebody using the device and use the Webex App app to control the device. For more information, see [Webex App | Call Someone from a Direct Message](https://help.webex.com/article/ge0ybp).
- Answer a call from somebody using the device and use the Webex App app to control the device. For more information, see [Answer a Call in Cisco Webex App](https://help.webex.com/article/n19ix1l).
- Start a meeting in a Webex Appspace using the device and use the Webex App app to control the device. For more information, see [Start an Instant Cisco Webex App Meeting from a Space](https://help.webex.com/article/nyrqyyw).
- Share their screen locally, however they can't share their computer audio or optimize video. For more information, see [Share Content Wirelessly from Webex App to Cisco Webex Devices](https://help.webex.com/article/pceb9t).

| 1 | From the customer view in [https:/​/​admin.webex.com](https://admin.webex.com/), go to Settings.          |
| - | --------------------------------------------------------------------------------------------------------- |
| 2 | Scroll to Device Discovery and check Allow the Webex App app to connect to on-premises registered device. |

## Network Requirements for On-Premises Devices

### Proximity in Networks

When Intelligent Proximity services are enabled on a room or desk device, an inaudible ultrasonic sound token is played through the room or desk device loudspeakers. Webex App records this token through the integrated microphone. The token contains information on how to connect to the room or desk device over the network. 
...
If the room or desk device is configured behind a Network Address Translation (NAT), Webex App won't connect to the room or desk device. However, the computer or mobile device can connect to the room or desk device when the IPv4 address of the room or desk device is reachable from behind the client NAT.

## Deployment for On-Premises Devices Connecting to Cisco Webex Cloud for Calls

When you use a Webex App app that is paired to an on-premises video device to make a call, that call is made using the video device and routed using the Cisco Webex Cloud. The video device must be able to call outside of your local network. 

If your deployment already includes a firewall traversal solution such as Expressway or VCS and the URIs to \*.webex.com and \*.ciscospark.com can be dialed, then no further action should be needed to allow calling to work. If your deployment doesn't include a firewall traversal solution, then the following guide provides guidance on how to deploy it, [Cisco Webex Meetings Enterprise Deployment Guide for Video Device-Enabled Meetings](https://www.cisco.com/c/dam/en/us/td/docs/collaboration/webex%5Fcenters/esp/Cisco%5FWebex%5FMeetings%5FEnterprise%5FDeployment%5FGuide%5FVideo%5FDevice%5FEnabled%5FMeetings.pdf).
...
There are differences in how the Webex App app connects to cloud-registered and on-premises devices. When connecting to an on-premises device, the content that is shared between Webex App and the room or desk device is always encrypted. However, we don’t enforce certificate verification when an HTTPS session is established with an on-premises device. Verifying certificates would prevent any guest devices working and would be complex to deploy and maintain.
